...
export class Exporter {
  ...
  private async generateProjectMarkdown(
    ...
    for (const file of projectIndex.files.sort((a, b) =>
   ...
   * AIツール向けのコンテキストを生成
   */
  private async generateAIToolContext(
    projectIndex: ProjectIndex,
  ...
  private describeArchitecture(projectIndex: ProjectIndex): string {
    // フォルダー構造から推測
    const hasWebview = projectIndex.files.some((f) =>
      f.filePath.includes("webview"),
    );
    const hasCore = projectIndex.files.some((f) => f.filePath.includes("core"));
    const hasParsers = projectIndex.files.some((f) =>
      f.filePath.includes("parser"),
    );
