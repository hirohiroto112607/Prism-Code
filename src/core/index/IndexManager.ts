...
export class IndexManager {
  ...
  private async createPrismcodeStructure(): Promise<void> {
    ...
    await fs.mkdir(path.join(this.prismcodeDir, "cache", "ir-cache"), {
      recursive: true,
    });
    await fs.mkdir(path.join(this.prismcodeDir, "analysis"), {
      recursive: true,
    });
    await fs.mkdir(path.join(this.prismcodeDir, "exports", "markdown"), {
      recursive: true,
    });
    await fs.mkdir(path.join(this.prismcodeDir, "exports", "ai-context"), {
      recursive: true,
    });
...
`;
  ...
  async isFileCached(filePath: string): Promise<boolean> {
    ...
    const entry = this.projectIndex.files.find(
  ...
  private updateMetadata(): void {
    ...
    const metadata = {
      ...
      totalLines: this.projectIndex.files.reduce(
        (sum, f) => sum + f.lineCount,
        0,
      ),
      totalFunctions: this.projectIndex.files.reduce(
