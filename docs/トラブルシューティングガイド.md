...
## 🤖 AI解析（Gemini / OpenAI）関連の問題

### 問題A: Gemini APIキーが未設定

**症状**: 「Gemini APIキーが設定されていません」というエラーが表示される。

**解決方法**:

1. コマンドパレット → 「設定を開く」
2. `prismcode.geminiApiKey` に取得したAPIキーを入力
3. 再度可視化コマンドを実行

### 問題B: 429 クォータ超過エラー

**症状**: 「Gemini APIの無料枠クォータを超過しました」というエラーが表示される。

**解決方法**:

- 表示されるアクションボタン「`gemini-2.0-flash-lite` に切り替える」をクリック
- または設定で `prismcode.geminiModel` を `gemini-2.0-flash-lite` に変更
...
### 問題C: GeminiのレスポンスがIR形式に準拠していない

**症状**: 「GeminiのレスポンスがIR形式に準拠していません」というエラーが表示される。

**原因**: コードが複雑すぎてGeminiがスキーマに沿ったJSONを生成できなかった。

**解決方法**:

1. 対象ファイルを小さな関数に分割してから再試行
2. より高精度なモデル（`gemini-2.0-flash`, `gemini-2.5-pro` 等）に切り替える
3. ts-morphパスにフォールバック: 設定で `prismcode.useGeminiAnalysis` を `false` にする

### 問題D: Gemini解析と静的解析の結果が異なる

**症状**: AIパスとts-morphパスでフローチャートの形が異なる。

**理由**: これは正常な動作です。

- ts-morphパス: ASTを機械的に変換するため構文に忠実
- Gemini AIパス: コードの**意図**を解釈するため、より意味的なフローになる

用途に応じてどちらを使うか選択してください。
