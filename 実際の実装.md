# LogicFlowBridge 実装詳細

## 1. システムアーキテクチャ

### 1.1 全体構成

```
┌─────────────────────────────────────────────────────────────┐
│                    VSCode Extension                          │
│  ┌───────────────────────────────────────────────────────┐  │
│  │              Extension Host (Node.js)                  │  │
│  │  ┌─────────────┐  ┌──────────────┐  ┌─────────────┐  │  │
│  │  │   Command    │  │ WebView Panel│  │  Language   │  │  │
│  │  │   Handler    │  │   Manager    │  │   Client    │  │  │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬──────┘  │  │
│  └─────────┼──────────────────┼──────────────────┼─────────┘  │
│            │                  │                  │            │
│            ▼                  ▼                  ▼            │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │                  Core Engine (疎結合)                    │ │
│  │  ┌───────────────────────────────────────────────────┐  │ │
│  │  │         Parser Interface (抽象層)                  │  │ │
│  │  │  ┌──────────┐  ┌──────────┐  ┌──────────┐        │  │ │
│  │  │  │TypeScript│  │   Java   │  │  Python  │ (将来) │  │ │
│  │  │  │  Parser  │  │  Parser  │  │  Parser  │        │  │ │
│  │  │  └────┬─────┘  └────┬─────┘  └────┬─────┘        │  │ │
│  │  └───────┼─────────────┼─────────────┼──────────────┘  │ │
│  │          │             │             │                  │ │
│  │          └─────────────┴─────────────┘                  │ │
│  │                       ▼                                  │ │
│  │  ┌──────────────────────────────────────────────────┐   │ │
│  │  │      IR (Intermediate Representation)            │   │ │
│  │  │  - 言語非依存の共通フォーマット                    │   │ │
│  │  │  - ノード、エッジ、制御フロー、データフロー        │   │ │
│  │  └────────────────────┬─────────────────────────────┘   │ │
│  │                       ▼                                  │ │
│  │  ┌──────────────────────────────────────────────────┐   │ │
│  │  │    Visualization Engine                          │   │ │
│  │  │  - IRをフローチャート要素に変換                   │   │ │
│  │  │  - レイアウトアルゴリズム                         │   │ │
│  │  └────────────────────┬─────────────────────────────┘   │ │
│  └─────────────────────────┼───────────────────────────────┘ │
│                            │                                  │
│            ┌───────────────┴───────────────┐                 │
│            ▼                               ▼                 │
│  ┌──────────────────┐           ┌──────────────────┐        │
│  │  WebView (React) │           │   Data Models    │        │
│  │  - React Flow    │           │   - AST Cache    │        │
│  │  - フローチャート │           │   - IR Cache     │        │
│  │    レンダリング   │           │                  │        │
│  └──────────────────┘           └──────────────────┘        │
└─────────────────────────────────────────────────────────────┘
```

### 1.2 レイヤー構造（疎結合設計）

| レイヤー | 責務 | 技術 |
|---------|------|------|
| **Presentation** | UI表示、ユーザー操作 | React, React Flow, VSCode WebView |
| **Application** | コマンド処理、状態管理 | VSCode Extension API |
| **Domain** | ビジネスロジック、変換処理 | TypeScript (Pure Logic) |
| **Infrastructure** | パーサー実装、ファイルI/O | ts-morph, Node.js APIs |

---

## 2. データフロー（Phase 1: 読み取り専用）

```
┌──────────────┐
│ TypeScript   │
│ Source Code  │
└──────┬───────┘
       │
       ▼
┌──────────────────────┐
│  Parser Interface    │  ← 抽象層（将来的に他言語対応）
│  parse(code): AST    │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  TypeScript Parser   │  ← ts-morph使用
│  (Concrete Impl)     │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│    AST (抽象構文木)   │
│  - FunctionDecl      │
│  - IfStatement       │
│  - ForStatement等    │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│   IR Transformer     │  ← 言語非依存の中間表現へ変換
│   transform(ast): IR │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│   IR (JSON形式)      │
│  {                   │
│    nodes: [...],     │
│    edges: [...],     │
│    metadata: {...}   │
│  }                   │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  Flow Generator      │  ← React Flow形式に変換
│  generate(ir): Flow  │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│   React Flow Data    │
│  {                   │
│    nodes: [...],     │
│    edges: [...]      │
│  }                   │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│   WebView Render     │
│   (フローチャート表示)|
└──────────────────────┘
```

---

## 3. コアコンポーネント設計

### 3.1 Parser Interface（疎結合のための抽象層）

```typescript
// src/core/parser/IParser.ts
export interface IParser {
  /**
   * ソースコードを解析してASTを返す
   */
  parse(code: string, filePath?: string): AST;

  /**
   * サポートする言語
   */
  getSupportedLanguage(): string;

  /**
   * ファイル拡張子でパーサーを識別
   */
  getSupportedExtensions(): string[];
}

// src/core/parser/AST.ts
export interface AST {
  type: 'Program';
  body: ASTNode[];
  sourceFile?: string;
}

export type ASTNode =
  | FunctionNode
  | ClassNode
  | IfStatementNode
  | ForStatementNode
  | WhileStatementNode
  | VariableDeclarationNode
  | ReturnStatementNode
  | ExpressionNode;

export interface FunctionNode {
  type: 'FunctionDeclaration';
  name: string;
  parameters: Parameter[];
  body: ASTNode[];
  returnType?: string;
  location: SourceLocation;
}

export interface IfStatementNode {
  type: 'IfStatement';
  condition: ExpressionNode;
  thenBranch: ASTNode[];
  elseBranch?: ASTNode[];
  location: SourceLocation;
}

export interface SourceLocation {
  start: { line: number; column: number };
  end: { line: number; column: number };
}

// ... 他のノード型
```

### 3.2 TypeScript Parser実装

```typescript
// src/parsers/typescript/TypeScriptParser.ts
import { Project, SourceFile } from 'ts-morph';
import { IParser, AST, ASTNode } from '../../core/parser/IParser';

export class TypeScriptParser implements IParser {
  private project: Project;

  constructor() {
    this.project = new Project({
      compilerOptions: {
        target: ts.ScriptTarget.Latest,
      },
    });
  }

  parse(code: string, filePath?: string): AST {
    const sourceFile = this.project.createSourceFile(
      filePath || 'temp.ts',
      code
    );

    return {
      type: 'Program',
      body: this.parseSourceFile(sourceFile),
      sourceFile: filePath,
    };
  }

  getSupportedLanguage(): string {
    return 'TypeScript';
  }

  getSupportedExtensions(): string[] {
    return ['.ts', '.tsx'];
  }

  private parseSourceFile(sourceFile: SourceFile): ASTNode[] {
    const nodes: ASTNode[] = [];

    // 関数宣言の解析
    sourceFile.getFunctions().forEach((fn) => {
      nodes.push(this.parseFunctionDeclaration(fn));
    });

    // クラスの解析
    sourceFile.getClasses().forEach((cls) => {
      nodes.push(this.parseClassDeclaration(cls));
    });

    return nodes;
  }

  private parseFunctionDeclaration(fn: any): FunctionNode {
    // 実装詳細
  }

  // ... 他のパース処理
}
```

### 3.3 IR（中間表現）設計

```typescript
// src/core/ir/IR.ts
/**
 * 言語非依存の中間表現
 * すべての言語からこの形式に変換される
 */
export interface IR {
  version: string;
  metadata: IRMetadata;
  nodes: IRNode[];
  edges: IREdge[];
}

export interface IRMetadata {
  sourceLanguage: string;
  sourceFile: string;
  timestamp: number;
}

export type IRNode =
  | IRFunctionNode
  | IRControlFlowNode
  | IRDataNode
  | IRStartNode
  | IREndNode;

export interface IRFunctionNode {
  id: string;
  type: 'function';
  name: string;
  parameters: string[];
  returnType?: string;
  body: string[];  // 子ノードのID
  location: SourceLocation;
}

export interface IRControlFlowNode {
  id: string;
  type: 'if' | 'for' | 'while' | 'switch';
  condition?: string;
  branches: {
    [key: string]: string[];  // ブランチ名 → ノードID[]
  };
  location: SourceLocation;
}

export interface IRDataNode {
  id: string;
  type: 'variable' | 'constant' | 'parameter';
  name: string;
  dataType?: string;
  initialValue?: string;
  location: SourceLocation;
}

export interface IRStartNode {
  id: string;
  type: 'start';
  label: string;
}

export interface IREndNode {
  id: string;
  type: 'end';
  label: string;
}

export interface IREdge {
  id: string;
  source: string;  // ノードID
  target: string;  // ノードID
  label?: string;  // "true", "false", "loop" 等
  type: 'control' | 'data';
}
```

### 3.4 IR Transformer

```typescript
// src/core/transformer/IRTransformer.ts
import { AST } from '../parser/IParser';
import { IR, IRNode, IREdge } from '../ir/IR';

export class IRTransformer {
  private nodeIdCounter = 0;
  private edgeIdCounter = 0;

  transform(ast: AST, metadata: { language: string; file: string }): IR {
    const nodes: IRNode[] = [];
    const edges: IREdge[] = [];

    ast.body.forEach((astNode) => {
      this.transformNode(astNode, nodes, edges);
    });

    return {
      version: '1.0.0',
      metadata: {
        sourceLanguage: metadata.language,
        sourceFile: metadata.file,
        timestamp: Date.now(),
      },
      nodes,
      edges,
    };
  }

  private transformNode(
    astNode: ASTNode,
    nodes: IRNode[],
    edges: IREdge[]
  ): string {
    // ノードタイプに応じて変換
    switch (astNode.type) {
      case 'FunctionDeclaration':
        return this.transformFunction(astNode, nodes, edges);
      case 'IfStatement':
        return this.transformIf(astNode, nodes, edges);
      // ... 他のケース
      default:
        throw new Error(`Unknown node type: ${astNode.type}`);
    }
  }

  private transformFunction(
    fn: FunctionNode,
    nodes: IRNode[],
    edges: IREdge[]
  ): string {
    const nodeId = this.generateNodeId();

    // 開始ノード
    const startId = this.generateNodeId();
    nodes.push({
      id: startId,
      type: 'start',
      label: `Start: ${fn.name}`,
    });

    // 関数本体の処理
    const bodyIds = fn.body.map((stmt) =>
      this.transformNode(stmt, nodes, edges)
    );

    // 終了ノード
    const endId = this.generateNodeId();
    nodes.push({
      id: endId,
      type: 'end',
      label: `End: ${fn.name}`,
    });

    // エッジの接続
    edges.push({
      id: this.generateEdgeId(),
      source: startId,
      target: bodyIds[0] || endId,
      type: 'control',
    });

    // body間の接続
    for (let i = 0; i < bodyIds.length - 1; i++) {
      edges.push({
        id: this.generateEdgeId(),
        source: bodyIds[i],
        target: bodyIds[i + 1],
        type: 'control',
      });
    }

    if (bodyIds.length > 0) {
      edges.push({
        id: this.generateEdgeId(),
        source: bodyIds[bodyIds.length - 1],
        target: endId,
        type: 'control',
      });
    }

    return startId;
  }

  private transformIf(
    ifNode: IfStatementNode,
    nodes: IRNode[],
    edges: IREdge[]
  ): string {
    const nodeId = this.generateNodeId();

    const thenIds = ifNode.thenBranch.map((stmt) =>
      this.transformNode(stmt, nodes, edges)
    );

    const elseIds = ifNode.elseBranch
      ? ifNode.elseBranch.map((stmt) => this.transformNode(stmt, nodes, edges))
      : [];

    nodes.push({
      id: nodeId,
      type: 'if',
      condition: this.expressionToString(ifNode.condition),
      branches: {
        then: thenIds,
        else: elseIds,
      },
      location: ifNode.location,
    });

    return nodeId;
  }

  private generateNodeId(): string {
    return `node_${this.nodeIdCounter++}`;
  }

  private generateEdgeId(): string {
    return `edge_${this.edgeIdCounter++}`;
  }
}
```

### 3.5 Flow Generator（React Flow形式への変換）

```typescript
// src/core/visualization/FlowGenerator.ts
import { IR, IRNode, IREdge } from '../ir/IR';
import { Node, Edge } from 'reactflow';

export class FlowGenerator {
  generate(ir: IR): { nodes: Node[]; edges: Edge[] } {
    const nodes: Node[] = ir.nodes.map((irNode) =>
      this.convertToReactFlowNode(irNode)
    );

    const edges: Edge[] = ir.edges.map((irEdge) =>
      this.convertToReactFlowEdge(irEdge)
    );

    return { nodes, edges };
  }

  private convertToReactFlowNode(irNode: IRNode): Node {
    const baseNode = {
      id: irNode.id,
      position: { x: 0, y: 0 },  // レイアウトアルゴリズムで後で計算
      data: {},
    };

    switch (irNode.type) {
      case 'function':
        return {
          ...baseNode,
          type: 'custom',
          data: {
            label: irNode.name,
            nodeType: 'function',
            parameters: irNode.parameters,
            returnType: irNode.returnType,
          },
        };

      case 'if':
        return {
          ...baseNode,
          type: 'custom',
          data: {
            label: irNode.condition || 'if',
            nodeType: 'decision',
            condition: irNode.condition,
          },
        };

      case 'start':
        return {
          ...baseNode,
          type: 'input',
          data: {
            label: irNode.label,
          },
        };

      case 'end':
        return {
          ...baseNode,
          type: 'output',
          data: {
            label: irNode.label,
          },
        };

      default:
        return baseNode;
    }
  }

  private convertToReactFlowEdge(irEdge: IREdge): Edge {
    return {
      id: irEdge.id,
      source: irEdge.source,
      target: irEdge.target,
      label: irEdge.label,
      type: irEdge.type === 'control' ? 'smoothstep' : 'default',
    };
  }
}
```

---

## 4. VSCode拡張機能の構造

### 4.1 ディレクトリ構造

```
logicflow-bridge/
├── package.json
├── tsconfig.json
├── .vscode/
│   └── launch.json
├── src/
│   ├── extension.ts                 # エントリーポイント
│   ├── core/                        # コアロジック（言語非依存）
│   │   ├── parser/
│   │   │   ├── IParser.ts           # パーサーインターフェース
│   │   │   └── AST.ts               # AST型定義
│   │   ├── ir/
│   │   │   └── IR.ts                # 中間表現の型定義
│   │   ├── transformer/
│   │   │   └── IRTransformer.ts     # AST → IR 変換
│   │   └── visualization/
│   │       ├── FlowGenerator.ts     # IR → ReactFlow変換
│   │       └── LayoutEngine.ts      # レイアウト計算
│   ├── parsers/                     # 言語別パーサー実装
│   │   ├── typescript/
│   │   │   └── TypeScriptParser.ts
│   │   ├── java/                    # 将来的に追加
│   │   └── ParserFactory.ts         # パーサーの選択
│   ├── commands/                    # VSCodeコマンド
│   │   └── visualizeCommand.ts
│   ├── webview/                     # WebView関連
│   │   ├── WebViewProvider.ts
│   │   └── messaging.ts
│   └── utils/
│       └── logger.ts
├── webview-ui/                      # React UI
│   ├── package.json
│   ├── src/
│   │   ├── App.tsx
│   │   ├── components/
│   │   │   ├── FlowChart.tsx        # React Flow表示
│   │   │   └── CustomNodes/         # カスタムノード
│   │   │       ├── FunctionNode.tsx
│   │   │       ├── IfNode.tsx
│   │   │       └── LoopNode.tsx
│   │   └── hooks/
│   │       └── useFlowData.ts
│   └── vite.config.ts
└── media/                           # アイコン等
```

### 4.2 Extension Entry Point

```typescript
// src/extension.ts
import * as vscode from 'vscode';
import { VisualizeCommand } from './commands/visualizeCommand';
import { WebViewProvider } from './webview/WebViewProvider';

export function activate(context: vscode.ExtensionContext) {
  console.log('LogicFlowBridge is now active!');

  // WebView Provider の登録
  const provider = new WebViewProvider(context.extensionUri);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      'logicflowbridge.flowView',
      provider
    )
  );

  // コマンドの登録
  const visualizeCommand = new VisualizeCommand(provider);
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'logicflowbridge.visualize',
      () => visualizeCommand.execute()
    )
  );
}

export function deactivate() {}
```

### 4.3 Visualize Command

```typescript
// src/commands/visualizeCommand.ts
import * as vscode from 'vscode';
import { ParserFactory } from '../parsers/ParserFactory';
import { IRTransformer } from '../core/transformer/IRTransformer';
import { FlowGenerator } from '../core/visualization/FlowGenerator';
import { WebViewProvider } from '../webview/WebViewProvider';

export class VisualizeCommand {
  constructor(private webviewProvider: WebViewProvider) {}

  async execute() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showErrorMessage('アクティブなエディタがありません');
      return;
    }

    const document = editor.document;
    const code = document.getText();
    const filePath = document.fileName;

    try {
      // 1. パーサーを取得（拡張子から自動判定）
      const parser = ParserFactory.getParser(filePath);

      // 2. コードを解析してASTを生成
      const ast = parser.parse(code, filePath);

      // 3. ASTをIRに変換
      const transformer = new IRTransformer();
      const ir = transformer.transform(ast, {
        language: parser.getSupportedLanguage(),
        file: filePath,
      });

      // 4. IRをReact Flow形式に変換
      const flowGenerator = new FlowGenerator();
      const flowData = flowGenerator.generate(ir);

      // 5. WebViewに送信
      this.webviewProvider.sendFlowData(flowData);

      vscode.window.showInformationMessage('フローチャートを生成しました');
    } catch (error) {
      vscode.window.showErrorMessage(
        `エラーが発生しました: ${error.message}`
      );
    }
  }
}
```

### 4.4 Parser Factory（疎結合のための工場パターン）

```typescript
// src/parsers/ParserFactory.ts
import { IParser } from '../core/parser/IParser';
import { TypeScriptParser } from './typescript/TypeScriptParser';
// import { JavaParser } from './java/JavaParser';  // 将来的に追加

export class ParserFactory {
  private static parsers: IParser[] = [
    new TypeScriptParser(),
    // new JavaParser(),  // 将来的に追加
  ];

  static getParser(filePath: string): IParser {
    const extension = filePath.substring(filePath.lastIndexOf('.'));

    for (const parser of this.parsers) {
      if (parser.getSupportedExtensions().includes(extension)) {
        return parser;
      }
    }

    throw new Error(`サポートされていないファイル形式です: ${extension}`);
  }

  static registerParser(parser: IParser): void {
    this.parsers.push(parser);
  }
}
```

---

## 5. WebView (React UI)

### 5.1 FlowChart Component

```typescript
// webview-ui/src/components/FlowChart.tsx
import React, { useCallback } from 'react';
import ReactFlow, {
  Node,
  Edge,
  Controls,
  Background,
  MiniMap,
  useNodesState,
  useEdgesState,
} from 'reactflow';
import 'reactflow/dist/style.css';

import FunctionNode from './CustomNodes/FunctionNode';
import IfNode from './CustomNodes/IfNode';

const nodeTypes = {
  function: FunctionNode,
  decision: IfNode,
};

interface FlowChartProps {
  initialNodes: Node[];
  initialEdges: Edge[];
}

export const FlowChart: React.FC<FlowChartProps> = ({
  initialNodes,
  initialEdges,
}) => {
  const [nodes, setNodes, onNodesChange] = useNodesState(initialNodes);
  const [edges, setEdges, onEdgesChange] = useEdgesState(initialEdges);

  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    console.log('Node clicked:', node);
    // 将来的にはコードへジャンプする機能を追加
  }, []);

  return (
    <div style={{ width: '100%', height: '100vh' }}>
      <ReactFlow
        nodes={nodes}
        edges={edges}
        onNodesChange={onNodesChange}
        onEdgesChange={onEdgesChange}
        onNodeClick={onNodeClick}
        nodeTypes={nodeTypes}
        fitView
      >
        <Controls />
        <MiniMap />
        <Background variant="dots" gap={12} size={1} />
      </ReactFlow>
    </div>
  );
};
```

### 5.2 Custom Node Example

```typescript
// webview-ui/src/components/CustomNodes/FunctionNode.tsx
import React from 'react';
import { Handle, Position } from 'reactflow';

interface FunctionNodeData {
  label: string;
  parameters: string[];
  returnType?: string;
}

const FunctionNode: React.FC<{ data: FunctionNodeData }> = ({ data }) => {
  return (
    <div
      style={{
        padding: '10px',
        border: '2px solid #1a73e8',
        borderRadius: '8px',
        background: '#fff',
        minWidth: '150px',
      }}
    >
      <Handle type="target" position={Position.Top} />

      <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>
        📦 {data.label}
      </div>

      <div style={{ fontSize: '12px', color: '#666' }}>
        ({data.parameters.join(', ')})
        {data.returnType && `: ${data.returnType}`}
      </div>

      <Handle type="source" position={Position.Bottom} />
    </div>
  );
};

export default FunctionNode;
```

---

## 6. Phase 1 実装計画

### 6.1 実装ステップ

#### Step 1: プロジェクトセットアップ（1日）
- [ ] VSCode拡張機能のボイラープレート作成
- [ ] TypeScript環境構築
- [ ] React WebView環境構築（Vite）
- [ ] 開発用のlaunch設定

#### Step 2: コア設計実装（3-4日）
- [ ] IParser インターフェース定義
- [ ] AST型定義
- [ ] IR型定義
- [ ] TypeScriptParser実装（基本的な関数とif文のみ）
- [ ] IRTransformer実装（基本変換）

#### Step 3: ビジュアライゼーション（2-3日）
- [ ] FlowGenerator実装
- [ ] 簡易レイアウトアルゴリズム（Dagre使用）
- [ ] React FlowChart コンポーネント
- [ ] カスタムノード（Function, If）

#### Step 4: VSCode統合（2日）
- [ ] Visualizeコマンド実装
- [ ] WebViewProvider実装
- [ ] メッセージング（Extension ↔ WebView）

#### Step 5: テスト＆デバッグ（2日）
- [ ] サンプルコードで動作確認
- [ ] エラーハンドリング
- [ ] パフォーマンス最適化

### 6.2 MVP機能リスト

✅ **サポートする構文（TypeScript）**
- 関数宣言（function, arrow function）
- if/else文
- for/while ループ
- 変数宣言
- return文

✅ **フローチャート要素**
- 開始/終了ノード
- プロセスノード（関数実行）
- 判定ノード（if文）
- ループノード（for/while）

✅ **UI機能**
- フローチャート表示
- ズーム/パン
- ミニマップ
- ノードクリックでハイライト

❌ **Phase 1では対象外**
- 双方向編集
- クラスの詳細解析
- 複雑な式の評価
- 他言語対応

---

## 7. 開発環境セットアップ

### 7.1 必要なツール
```bash
# Node.js (v18+)
node --version

# VSCode Extension開発用
npm install -g yo generator-code

# プロジェクト生成
yo code
# → New Extension (TypeScript) を選択
```

### 7.2 依存パッケージ

**Extension側（package.json）**
```json
{
  "dependencies": {
    "ts-morph": "^20.0.0"
  },
  "devDependencies": {
    "@types/vscode": "^1.85.0",
    "@types/node": "^20.0.0",
    "typescript": "^5.3.0",
    "esbuild": "^0.19.0"
  }
}
```

**WebView UI側（webview-ui/package.json）**
```json
{
  "dependencies": {
    "react": "^18.2.0",
    "react-dom": "^18.2.0",
    "reactflow": "^11.10.0",
    "dagre": "^0.8.5"
  },
  "devDependencies": {
    "@vitejs/plugin-react": "^4.2.0",
    "vite": "^5.0.0",
    "typescript": "^5.3.0"
  }
}
```

### 7.3 ビルド構成

```json
// tsconfig.json (Extension)
{
  "compilerOptions": {
    "module": "commonjs",
    "target": "ES2020",
    "outDir": "out",
    "lib": ["ES2020"],
    "sourceMap": true,
    "rootDir": "src",
    "strict": true,
    "esModuleInterop": true
  }
}
```

---

## 8. 次のステップ

### Phase 1完了後の拡張（Phase 2）
1. **インタラクティブ性の追加**
   - ノードクリックでコードへジャンプ
   - コード編集時の自動更新

2. **より複雑な構文対応**
   - クラス、メソッド
   - try-catch
   - switch文
   - 非同期処理（async/await）

3. **レイアウト改善**
   - より洗練されたレイアウトアルゴリズム
   - ズームレベルに応じた詳細度調整

### Phase 3: 双方向編集
1. ノード編集によるコード生成
2. AI統合（Gemini API）
3. リアルタイム同期

---

## 9. 技術的な課題と解決策

| 課題 | 解決策 |
|------|--------|
| 複雑なコードの解析精度 | ts-morphの詳細なAPIを活用、段階的にサポート拡大 |
| レイアウトアルゴリズムの最適化 | Dagreライブラリを使用、後で独自実装 |
| パフォーマンス（大規模ファイル） | 差分更新、遅延読み込み、Web Worker活用 |
| 他言語への拡張 | IParserインターフェースで抽象化済み |

---

以上が、LogicFlowBridge Phase 1の詳細実装計画です。
